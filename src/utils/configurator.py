...
class Config(object):
    ...
    def _load_dataset_model_config(self, config_dict):
        ...
        # 确保路径是正确的
        overall_config = os.path.join(cur_dir, "overall.yaml")
        dataset_config = os.path.join(cur_dir, "dataset", "{}.yaml".format(config_dict['dataset']))
        model_config = os.path.join(cur_dir, "model", "{}.yaml".format(config_dict['model'].upper()))
        
        print(f"整体配置路径: {overall_config}, 存在: {os.path.exists(overall_config)}")
        print(f"数据集配置路径: {dataset_config}, 存在: {os.path.exists(dataset_config)}")
        print(f"模型配置路径: {model_config}, 存在: {os.path.exists(model_config)}")
        
        file_list.append(overall_config)
        file_list.append(dataset_config)
        file_list.append(model_config)
    ...
    def _set_default_parameters(self):
        ...
        # Debug: print hyper_parameters to see what's happening
        print(f"DEBUG: hyper_parameters after _set_default_parameters: {self.final_config_dict['hyper_parameters']}")

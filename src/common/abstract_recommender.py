...
class GeneralRecommender(AbstractRecommender):
    ...
    def __init__(self, config, dataloader):
        ...
        if not config['end2end'] and config['is_multimodal_model']:
            # 正确计算数据集路径
            dataset_path = os.path.join(os.path.dirname(os.getcwd()), 'data', config['dataset'])
            print(f"特征文件路径: {dataset_path}")
            ...
            print(f"图像特征文件: {v_feat_file_path}, 存在: {os.path.exists(v_feat_file_path)}")
            print(f"文本特征文件: {t_feat_file_path}, 存在: {os.path.exists(t_feat_file_path)}")
